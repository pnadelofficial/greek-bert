...
def only_lookup() -> Language:
    ...
    lookups = Lookups()
    with open("assets/lemmas/lookup.json") as lookup_file:
...
def main() -> None:
    ...
    results = {}
    test_path = "corpus/test.spacy"
    results[test_path] = {}
    for diacritics in ["with", "without"]:
        print(f"{diacritics} diacritics:")
        results[test_path][f"{diacritics} diacritics:"] = {}
        for name, create_nlp in pipelines.items():
            ...
            results[test_path][f"{diacritics} diacritics:"][name] = scores

    with open("output/lemmatizers.json", "w") as out_file:
